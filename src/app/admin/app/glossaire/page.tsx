...
import GlossaireClient, { type GlossaryTerm } from '@/app/glossaire/GlossaireClient'
...
export default async function AdminGlossairePage() {
  let terms: GlossaryTerm[] = STATIC_TERMS

  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL ?? ''
  if (supabaseUrl.startsWith('http')) {
    const supabase = await createClient()
    const { data } = await supabase.from('glossary').select('id, term, definition, category').order('term', { ascending: true })
    if (data && data.length > 0) terms = data as GlossaryTerm[]
  }

  return (
    <div style={{ padding: '4px 0 60px', maxWidth: 900 }}>
      ...
      <div style={{ marginBottom: 28, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 14, color: '#616161', maxWidth: 500, margin: '0 auto', lineHeight: '22px' }}>
          Comprendre les termes techniques de la phytothérapie et de la botanique.
        </p>
        <div style={{ marginTop: 8, fontSize: 12, color: '#616161' }}>{terms.length} termes disponibles</div>
      </div>
